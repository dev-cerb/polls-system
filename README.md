...
- "Não iniciada." → A data atual ainda não atingiu a data de início da enquete.
- "Iniciada." → A enquete está disponível para votação, mas ainda não recebeu votos.
- "Em andamento." → A enquete está ativa e já possui votos registrados.
- "Finalizada." → A data atual ultrapassou a data de término da enquete.
...
Foi realizado o deploy da API e a mesma se encontra disponível através da seguinte URL:

`https://polls-system-rkrh.onrender.com`

### Acessando a documentação da API no Scalar

`https://polls-system-rkrh.onrender.com/docs`

### Endpoint WebSocket para Voto em tempo real

`wss://polls-system-rkrh.onrender.com/polls/:pollId/results`
